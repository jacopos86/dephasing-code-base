...
def test_program_parse():
    TESTS_DIR = os.environ['TESTS']
    yml_inp = str(TESTS_DIR) + "/1/input.yml"
    argv_list = ["--yml_inp", yml_inp]
    args = parse_args(argv_list)
    yml_inp_parse = str(TESTS_DIR) + "/1/input.yml"
...
def test_parameters():
    ...
    assert(pobj.unpert_dir, str(TESTS_DIR)+"/1/GS")
    assert(pobj.displ_poscar_dir[0], str(TESTS_DIR)+"/1/DISPLACEMENT-FILES-001")
    assert(pobj.displ_outcar_dir[0], str(TESTS_DIR)+"/1/DISPL-001")
