...
from pydephasing.compute_LR_elec_decoher import compute_elec_dephas, compute_elec_dephas_jdftx
    ...
    if calc_type1 == "RT":
        ...
        if calc_type2 == "vasp":
            ...
            if mpi.rank == mpi.root:
                ...
                log.info("\t USING JDFTx CALCULATION DATA")
                log.info("\n")
            T2_calc_handler = compute_elec_dephas_jdftx() # TO DO: implement JDFTx LR calculation
    else:
        if mpi.rank == mpi.root:
            log.info("\n")
            log.info("\t " + p.sep)
